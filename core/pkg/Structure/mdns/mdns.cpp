...
vector<string> *reciveSequence = NULL;
typedef struct
{
  const char *service;
  const char *hostname;
               ...
               const void *data, size_t size, size_t offset, size_t length, void *user_data)
{
  ...
  if (rtype == MDNS_RECORDTYPE_PTR)
  {
    ...
    if (reciveSequence != NULL)
      reciveSequence->push_back(namestr.str);
    if (MDNS_DEBUG)
  ...
  else if (rtype == MDNS_RECORDTYPE_A)
  {
    ...
    string add(addrstr.str);
    add = "A " + add;
    if (reciveSequence != NULL)
      reciveSequence->push_back(add);
    if (MDNS_DEBUG)
...
{
  int mode = 0;
  const char *service = "_bitNest.world.";
